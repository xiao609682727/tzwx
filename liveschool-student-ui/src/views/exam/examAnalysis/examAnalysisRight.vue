  ...
  <div class="co_r">
    <div :class="{'is_fixed':isFixed}">
      <div class="as_head as_head2">
        <p class="co_r_font"></p>
        <div class=" submit_btn" style="margin-top: 8px" @click="goBack()">查看报告</div>
      </div>
      <div class="ex_time">
        <div class="ex_h">
            ...
            <div class="as_ops_box" v-for="(paperMiddle,index) in examInfo.list" :key="index">
              <div class="as_ops_md"><p class="as_o_h"><span class="as_o_title">【{{ paperMiddle.name }}】</span><span
                ...
                <div class="as_c_num">
                  ...
                  <div id="1" class="as_ops_num" @click="go(qstmiddle.id)" :class="{'as_box_md_error':qstmiddle.qstStatus == 1,'as_box_md_right':qstmiddle.qstStatus == 2,'as_ops_bg':qstmiddle.signFlag}" v-for="(qstmiddle,index) in paperMiddle.questionArr" :key="index">{{ qstmiddle.num }}</div>
                </div>
              </div>
            </div>
          </div>
...
//如果被卷曲的高度大于吸顶元素到顶端位置 的距离
    ...
    goBack(){
      this.$router.push({ path: "/exam/examResult", query:{id:this.examRecord.id}});
    },
    go(i){
      this.$emit('go',i)
    },
