  ...
  <el-container>
    ...
    <el-dialog title="用户数据导入"
      ...
      <el-row v-if="errorMsg != ''">
        ...
        <el-col :span="20" style="color: #f56c6c">{{errorMsg}}</el-col>
      </el-row>
    </el-dialog>
    <el-dialog title="重置密码" width="30%" :visible.sync="dialogPwdFormVisible">
      <el-form :model="pwdForm">
        ...
        <el-form-item label="确认密码：" label-width="120px" >
          <el-input v-model="pwdForm.passwords" placeholder="请在一次输入密码" style="width: 70%;" show-password></el-input>
        </el-form-item>
      </el-form>
  ...
  export default {
          ...
          column: [
            {
              ...
              action: "/api/crazy-user/import-user"
            },
            {
              label: '模板下载',
              prop: 'excelTemplate',
    ...
    methods: {
      ...
      setPwd(){
        ...
        console.log("用户id"+this.pwdForm.id)
        updatePasswordNew(this.pwdForm.id,this.pwdForm.password,this.pwdForm.passwords).then(()=>{
          this.$message({
            type: "success",
            message: "操作成功!"
          });
      ...
      handleExport() {
        this.$confirm("是否导出用户数据?", "提示", {
          ...
          const searchForm = this.$refs.crud;
          console.log(searchForm)
          window.open(`/api/crazy-user/export-user?crazy-auth=${getToken()}`+
          `&account=`+this.search.account+`&realName=`+this.search.realName);
        });
      },
      handleTemplate() {
        window.open(`static/template/用户数据模板.xlsx`);
      },
