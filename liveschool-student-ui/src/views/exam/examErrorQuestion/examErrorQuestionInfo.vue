    ...
    <div class="main" style="min-height: 500px">
      <div class="container">
        <div class="rt_head">
          ...
          <span class="rt_num"><span class="cprimary">{{page.current}}</span>/{{page.pages}}</span>
          <div class="float_right">
            <div class="fr_box"  @click="goError">
              <img src="/static/img/exam/exam-fanhui.png" alt="">
              <p class="fr_b_font">返回错题本</p>
            </div>
            <div class="fr_box" @click="remove(list[0].id)" ><img  src="/static/img/exam/exam-delete.png"  alt="">
              <p class="fr_b_font">移除试题</p>
            </div>
          </div>
        </div>
...
import {getErrorquestion, removeErrorquestion,saveFavoritesqst,removeFavoritesqst} from "@/api/exam";
        ...
        layerid => {
          ...
          removeErrorquestion(id).then(res => {
            ...
            if(that.page.current == that.page.pages){
              this.$router.push({ path: "/exam/examErrorQuestionInfo",query:{current:parseInt(this.current) - 1} });
            }
            that.getList()
          })
