      ...
      <div class="allcourse-content js-course-list ">
        <div class="courseitem tl-item"  v-for="detail in detailList" :key="detail.id">
          ...
          <div class="info-box course-list">
            <div class="title">
              <span v-if="detail.trxorderType == 'COURSE'">点播</span>
              <span v-if="detail.trxorderType == 'LIVE'">直播</span>
              <span v-if="detail.trxorderType == 'PACKAGE'">班级</span>
              <span v-if="detail.trxorderType == 'LINECLASS'">面授</span>
              <router-link v-if="detail.trxorderType != 'LINECLASS'" :to="{path:'/course/courseInfo/'+detail.courseId}"  class="hd" target="_blank">{{detail.courseName}}</router-link>
              <router-link v-if="detail.trxorderType == 'LINECLASS'" :to="{path:'/lineDownCourse/lineInfo/'+detail.courseId}"  class="hd" target="_blank">{{detail.courseName}}</router-link>
            </div>
            <div class="catog-points" v-if="detail.trxorderType != 'LINECLASS'">
              <span class="i-left span-common ml10" >学习进度：{{detail.studyLearning}}（{{detail.studyKpoint}}节/共{{detail.studyKpoints}}节）</span>
              <!--<span class="i-mid span-common">用时 6分</span>
              <span class="i-right span-common">学习至2.12 单独安装eslint</span>-->
              <button disabled style="background-color:#E8E6E7" href="javascrpt:void(0)" class="btn-red continute-btn">已过期</button>
            </div>
            <div class="catog-points" v-if="detail.trxorderType == 'LINECLASS'&&detail.lineTeacherIs==3">
            ...
            <div class="catog-points" v-if="detail.trxorderType == 'LINECLASS'&&detail.lineTeacherIs==2">
              <button disabled style="background-color:#E8E6E7" href="javascrpt:void(0)" class="btn-red continute-btn">已过期</button>
              <span class="i-left span-common ml10" >未报名</span>
            </div>
            <div class="catog-points" v-if="detail.trxorderType != 'LINECLASS'">
              <span class="i-left span-common ml10">有效期：{{detail.authTime}}</span>
              <!--<span class="span-common">
                <a href="/u/5385180/notepad/1173?frm=allcourses">笔记
                  <i>0</i>
                </a>
              </span>
              <span class="i-mid span-common">
                <a href="/u/5385180/course/1173/codes?frm=allcourses">代码
                  <i>0</i>
                </a>
              </span>
              <span class="i-right span-common">
                <a href="/u/5385180/course/1173/questions?frm=allcourses">问答
                  <i>0</i>
                </a>
              </span>-->

              <!-- <div class="moco-ico-btn js-add-to-plan" style="position: absolute;right: 130px;" data-type="1" data-tid="1173">
                 加入课表
               </div>-->
              <!--<a @click="gotoPlay(detail.courseId)" href="javascrpt:void(0)" class="btn-red continute-btn">继续学习</a>-->
              <!--<router-link :to="{path:'/course/courseInfo/'+detail.courseId}" target="_blank" class="btn-red continute-btn">继续学习</router-link>-->
            </div>
...
<script>
  ...
  import {getTrxorderEnddetail} from "@/api/user";
  ...
  export default {
    ...
    methods: {
      ...
      lineTeacherPlay(courseId) {//去往面授详情。
        this.$router.push({ path: '/lineDownCourse/lineInfo/'+courseId })
        return
      },
      ...
      getList(){
        ...
        getTrxorderEnddetail(params).then(res =>{
